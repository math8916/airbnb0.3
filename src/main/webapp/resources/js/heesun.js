...
$('#pub_article').on('click','#m_bt_signup',function() {
		$(function() {
			$('#signup_email').keyup(
					function() {
							if (util.emailChecker($('#signup_email').val()) === 'yes'){
								...
								$('font[name=email_check]').html("올바른이메일주소입니다.").css('color', 'green');
							} else {
								$('font[name=email_check]').text('');
								$('font[name=email_check]').html("올바르지않은이메일주소입니다.").css('color', 'red');
							}
					}); 
		});
		$(function() {
		...
		$('#check_dup')	.click(function(e) {
			e.preventDefault();
					if (util.pwChecker($('#signup_pw').val()) === 'yes') {
						$.ajax({
							url : app.context()+ '/member/check_dup/'+ $('#id').val(),
							success : function(data) {
							if (data.flag === "TRUE") {
							$('#id_box').html('<input type="text" id="id" placeholder="'+ data.message+ '"><input type="button" id="id" value="다시 조회"/>');
								member.init();
							} else {
							$('#id_box').html('<input style= color:green; type="text" id="id" value="'+ data.temp
											+ '"><input type="button" id="id" value="그대로 사용"/>');
										member.init();
							$('#bt_join').click(function(e) {
										e.preventDefault();
									var join_info = {'id' : $('#id').val(),
													'pw' : $('#password').val(),
													'name' : $('#username').val(),
													'ssn' : $('#ssn').val(),
													'email' : $('#email').val(),
...
/*	+'<input style="width:100%;text-align:center" id="check_dup"  type="button" value="중복체크"/>'*/
	+'</div>'
...
var ACCOUNT = 
	...
	+'<a href="${context}/member/show/" title="View Profile"><img alt="Rachel" class="img-responsive" height="190" src='+app.img()+'/member/rachel.jpg title="rachel" width="190" /></a>'
	+'</div>'
	...
	+'<div style="padding-top:5%" id="inbox-container"'
